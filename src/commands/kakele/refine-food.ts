...
import {
  getHealthRefinementCost,
  getManaRefinementCost,
} from "../../misc/util/index.js";
import { createCanvas, type Canvas } from "canvas";
import { cachedImages } from "../../misc/canvas/index.js";

async function createRefineFoodImage(data: {
  value: number;
  mana: number;
  health: number;
}): Promise<Canvas> {
  const canvas = createCanvas(
    cachedImages.refineFood.width,
    cachedImages.refineFood.height,
  );
  const ctx = canvas.getContext("2d");

  ctx.drawImage(cachedImages.refineFood, 0, 0, canvas.width, canvas.height);

  ctx.font = "60px HelveticaBold";
...
export default new InteractionCommand({
  data: new SlashCommandBuilder()
    .setName("refine-food")
    .setDescription(
      "Exibe os custos de aprimoramento dos alimentos de acordo com seu bônus.",
    )
    .setDescriptionLocalizations({
      "es-ES":
        "Muestra los costos de mejora de los alimentos según su bonificación.",
      "pt-BR":
        "Exibe os custos de aprimoramento dos alimentos de acordo com seu bônus.",
