...
import { createItemListImage } from "../../misc/canvas/index.js";
import { getMarketList } from "../../misc/database/index.js";
import {
  type IOfferItemDocument,
  MarketplaceItem,
} from "../../database/schemas/marketplace-item.js";
                ...
                if (result.length > 0) {
                  if (result.length === 1) {
                    const emoji = client.kakeleEmojis.find(
                      (x) =>
                        x.name ===
                        result[0].item.name.replace(/[^A-Z0-9]/gi, ""),
                    );
